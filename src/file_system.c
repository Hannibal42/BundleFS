...
enum FSRESULT fs_mkfs(struct disk* disk)
{
    ...
    if(sector_count % 8 != 0) 
    /*TODO:This should never happen if you used the right numbers for the fs */
    	dif = dif - 1;
...
enum FSRESULT fs_delete(struct FILE_SYSTEM* fs, struct INODE* file)
{
	...
	inode_alloc_table = malloc(fs->sector_size * fs->inode_alloc_table_size);
	...
	disk_write(fs->disk, (char*) buffer, fs->inode_block + file->inode_offset,
	 1); 
 ...
 unsigned long size, unsigned int time_to_live, short custody)
{
	...
	unsigned int allocation_offset, inode_offset, temp_bit_count, startpadding;
